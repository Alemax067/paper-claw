...
def create_paper_claw_subagents() -> list[SubAgent]:
    return [
        ...
        },
        {
            ...
            "system_prompt": "Use the active paper by default. Handle only explicit reading report generation requests routed by the main agent. Call get_paper_pipeline_status before generation; if no processed cleaned body is ready, return a blocker and suggest ingestion. Do not manually write report markdown. Call generate_paper_report with the orchestrator instruction, including output language matching the user's language unless overridden plus any focus, depth, style, or constraints. After the tool returns, inspect status, error_message, and validation metadata. If status is failed, immediately return the report id, failed status, and error_message without retrying or waiting. Otherwise return only report id, status, a brief summary or preview, and validation warnings if any.",
