...
def create_paper_claw_subagents() -> list[SubAgent]:
    return [
        ...
        },
        {
            ...
            "description": "Prepare parseable artifacts for the active paper, then advance parsing and processing when artifacts are ready.",
            "system_prompt": "Use the active paper by default. First prepare artifacts: call get_paper_pipeline_status(include_metadata=False) and list_paper_artifacts to check for available source or PDF artifacts. If an available source artifact exists, report artifact_ready with next_step parse_tex_source. If only a PDF artifact exists, report artifact_ready with next_step parse_pdf. If no usable artifact exists, call get_paper_pipeline_status(include_metadata=True) and inspect identifiers, source_records, landing_page_url, best_pdf_url, DOI, and raw metadata. If any arXiv information exists, extract the arXiv id and call download_arxiv_paper_artifacts; the tool uses https://arxiv.org/src/{id} for source downloads and handles PDF download, rate limiting, and registration. Do not construct or use /e-print URLs. If no arXiv information exists, choose the most trustworthy PDF URL and call download_paper_pdf_from_url; never pass remote URLs to register_local_paper_pdf. If there is no usable URL or downloads fail, call mark_paper_artifact_upload_required and tell the user to upload a PDF or TeX source on the paper page before retrying. The model decides which tool to call from metadata; tools handle URL construction, download safety, artifact registration, and storage. After artifacts are ready, parse and process only when requested by the main workflow. Report artifact_ready, waiting_for_user_upload, ready, or failed with the blocking reason.",
